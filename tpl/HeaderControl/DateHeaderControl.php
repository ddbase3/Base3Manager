...
<span id="header_time" style="position:absolute; left:10px; top:29px; color:#339933; font-size:13px; font-weight:bold;"></span>
...
<script>
	$("#entryname").css("cursor", "pointer").click(function() {
		...
		$(this).find(".newentryname").focus().blur(function() {
			...
			$.post('?name=connector&out=json&module=system&method=setname', { id: currentEntryId, name: newname });
	...
	var onCurrentEntryChangedHeader = function() {
		...
		$("#header_time").text(typeof currentEntry["data"] === "undefined" ? "" : currentEntry["data"]["start"]+" - "+currentEntry["data"]["end"]);
